		...
		if sm.interactable.isNumberType(parents[1]) and not parents[1]:hasSteering() then                                                                -- power input, show correct decimal
			if #parents == 1 then
				if #children > 0 and children[1]:getType() ~= "scripted" and children[1]:getType() ~= "electricEngine" and children[1]:getType() ~= "gasEngine" then -- show bits as output
					...
					local s = self.dec
					[tostring(self.shape.color)]    -- gets correct decimal location in parent based on own color   -- decimal number
					...
					elseif (tostring(sm.shape.getColor(self.shape)) == "f5f071ff") then
						s = 0.00000001
						show = (tonumber(tostring((math.abs(show) / s)))) %
						1                               -- shifts digits after the decimal point 8 places to the left and %1
					end
		...
		if sm.interactable.isNumberType(parents[1]) and not parents[1]:hasSteering() then                                                                -- power input, show correct decimal
			if #parents == 1 then
				if #children > 0 and children[1]:getType() ~= "scripted" and children[1]:getType() ~= "electricEngine" and children[1]:getType() ~= "gasEngine" then -- show bits as output
					...
					local s = self.dec
					[tostring(sm.shape.getColor(self.shape))]    -- gets correct decimal location in parent based on own color   -- NumberBlock number
