...
WirelessBlock.connectionOutput = sm.interactable.connectionType.power + sm.interactable.connectionType.logic +
	...
	for k, v in pairs(parents) do
		...
		if v:hasSteering() or _isOldSeat or tostring(v:getShape():getShapeUuid()) == "ccaa33b6-e5bb-4edc-9329-b40f6efe2c9e" --[[orienter]] then
			...
			if sm.interactable.isNumberType(v) and tostring(sm.shape.getColor(v:getShape())) == "eeeeeeff" then
				...
				power = power +
				(sm.interactable.getValue(v) or v.power)     -- getValue will return values on the host but this doesn't matter, other clients don't need to know the value.
				sendsstuff = true
			end
		end
	end
