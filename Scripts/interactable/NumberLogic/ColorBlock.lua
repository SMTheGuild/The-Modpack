...
function ColorBlock.server_onFixedUpdate(self, dt)
	...
	elseif #parents >= 3 then
		...
		self.prevcolor = (function()
			local prevs = {}
			for k, v in pairs(parents) do table.insert(prevs, tostring(v:getShape().color)) end
			return prevs
		end)()
	...
	for k, v in pairs(parents) do
		...
		if _pColor == "eeeeeeff" then -- glow
			...
			self.network:sendToClients("client_giveError",
				"glow feature currently not supported untill the devs fix this.")
		elseif _pColor == "7f7f7fff" then -- reflection
			v:disconnect(self.interactable)
			self.network:sendToClients("client_giveError",
				"reflection feature currently not supported untill the devs fix this.")
		elseif _pColor == "222222ff" then -- specular
			v:disconnect(self.interactable)
			self.network:sendToClients("client_giveError",
				"specular feature currently not supported untill the devs fix this.")
		end
	end
