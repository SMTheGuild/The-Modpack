...
GasEngine.connectionInput = sm.interactable.connectionType.logic + sm.interactable.connectionType.power +
...
function GasEngine.server_onFixedUpdate(self, timeStep)
	...
	local hadInput = self.hasInput == nil and true or
	self.hasInput                                                --Pretend to have had input if nil to avoid starting engines at load
	...
	if self.dirtyClientTable then
		self.network:setClientData({ gearIdx = self.saved.gearIdx, engineHasFuel = self.hasFuel or useCreativeFuel, scrapOffset =
		self.scrapOffset })
		self.dirtyClientTable = false
	end
end
		...
		if level then
			if level.upgrade then
				...
				self.gui:setData("UpgradeInfo",
					{ Gears = nextLevel.gearCount - level.gearCount, Bearings = nextLevel.bearingCount -
					level.bearingCount, Efficiency = 1 })
...
function GasEngine.client_getAvailableParentConnectionCount(self, connectionType)
	if bit.band(connectionType, bit.bor(sm.interactable.connectionType.logic, sm.interactable.connectionType.power)) ~= 0 then
		return 1 -
		#self.interactable:getParents(bit.bor(sm.interactable.connectionType.logic, sm.interactable.connectionType.power))
	end
...
function GasEngine.cl_n_onUpgrade(self, upgrade)
	...
	if self.gui and self.gui:isActive() then
		...
		if level.upgrade then
			local nextLevel = EngineLevels[level.upgrade]
			self.gui:setData("UpgradeInfo",
				{ Gears = nextLevel.gearCount - level.gearCount, Bearings = nextLevel.bearingCount - level.bearingCount, Efficiency = 1 })
			self.gui:setIconImage("UpgradeIcon", sm.uuid.new(level.upgrade))
		else
			self.gui:setVisible("UpgradeIcon", false)
			self.gui:setData("UpgradeInfo", nil)
