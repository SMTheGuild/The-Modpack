...
function FlappyWing.server_init(self)
    ...
    for id, surface in pairs(self.data.surfaces) do
        if not surface.area or surface.area == 0 then
            sm.log.warning("Modpack wing with UUID=" ..
            self.shape.shapeUuid .. " has a useless surface with id=" .. id .. ".")
            surface = nil
        end
        ...
        surface.offsetModifierSin = sm.vec3.new(surface.offsetModifierSin[1], surface.offsetModifierSin[2],
            surface.offsetModifierSin[3])
        surface.offsetModifierCos = sm.vec3.new(surface.offsetModifierCos[1], surface.offsetModifierCos[2],
            surface.offsetModifierCos[3])
        surface.offsetModifierTan = sm.vec3.new(surface.offsetModifierTan[1], surface.offsetModifierTan[2],
            surface.offsetModifierTan[3])
    end
...
function FlappyWing.server_onFixedUpdate(self, timeStep)
    ...
    if parent then
        self.angle = self.data.angleLoop and parent.power or
        sm.util.clamp(parent.power, self.data.angleMin, self.data.angleMax)                                                      -- == 0 and -90 or 90
    else
        self.angle = 0
    end
