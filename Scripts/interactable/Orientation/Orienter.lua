...
sm.MODPACK_ORIENT_ADD_UNIT = function(name, unit_uuid, is_friendly)
	...
	if known_mobs[list][name][unit_uuid] == nil then
		...
		print("[Modpack] Unit \"" ..
		unit_uuid .. "\" named \"" .. name .. "\" has been successfully added into " .. list .. " units list!")
	end
end
...
function Orienter.sv_changeMode(self, params)
	...
	self.network:sendToClients('cl_setMode',
		{ mode = params.mode, uvIndex = Orienter.modetable[params.mode].savevalue - 1 })
end
...
function predictmove(self, mypos, direction, targetpos, localcalc) --MINE --FINAL
	...
	targetrelativepos = targetrelativepos + v * (5 / 40) + a *
	(5 / 40) ^ 2                                               -- targetrelativepos:normalize()*deltadistance*350
	...
	--distance reach fix:
	local supermagicmultiplier = math.min(100, -6.181747 + (140773.3 - -6.181747) /
	(1 + (distance / 0.005479015) ^ 0.7706603))
	local futuretarget = targetrelativepos +
	(v * t + (a - spuddrop) * t ^ 2 * sm.vec3.new(1, 1, (1 + distance / (supermagicmultiplier * 1666 * self.lead / 1))))
...
function Orienter.server_onFixedUpdate(self, dt)
	...
	if (mode >= 20 and mode <= 28) then
		local id = self:getFarmbot({ useexceptionlist = true, minrange = minrange, maxrange = maxrange, offset =
		blackinput, tryid = whiteinput, fbot_data = FarmbotDetectorModes[mode] })
	...
	elseif mode == 3 then -- orient player
		...
		local id = self:getplayer({ useexceptionlist = true, minrange = minrange, maxrange = maxrange, offset =
		blackinput, tryid = whiteinput })
		if id ~= 0 and isON then
			for k, v in pairs(parents) do
				local _sUuid = tostring(v:getShape():getShapeUuid())
				if (_sUuid == "add3acc6-a6fd-44e8-a384-a7a16ce13c81" or _sUuid == "4081ca6f-6b80-4c39-9e79-e1f747039bec") then
	...
	elseif mode == 5 and trackertrackers then -- orient block
		...
		local id = self:gettracker({ minrange = minrange, maxrange = maxrange, frequency = whiteinput, offset =
		blackinput, ignorejammers = false })
		if id == 0 then
	...
	elseif mode == 7 then -- orient camera
		...
		if id ~= 0 and isON then
			local hit, result = sm.physics.raycast(allplayers[id].character.worldPosition,
				allplayers[id].character.worldPosition + allplayers[id].character.direction * 2000)
	...
	elseif mode == 9 then -- orient player + tracker
		local closestplayer = self:getplayer({ useexceptionlist = true, minrange = minrange, maxrange = maxrange, offset =
		blackinput, tryid = whiteinput })
		local closesttracker = self:gettracker({ minrange = minrange, maxrange = maxrange, frequency = whiteinput, offset =
		blackinput, ignorejammers = false, colorignore = true })
		local closestmatchingtracker = self:gettracker({ minrange = minrange, maxrange = maxrange, frequency = whiteinput, offset =
		blackinput, ignorejammers = false })

		local closestdistance_player = (closestplayer ~= 0) and
		(self.shape:getWorldPosition() - allplayers[closestplayer].character.worldPosition):length() or math.huge
		local closestdistance_tracker = (closesttracker ~= 0) and
		(self.shape:getWorldPosition() - trackertrackers[closesttracker]:getTrackerShape().worldPosition):length() or
		math.huge
		local closestdistance_matchingtracker = (closestmatchingtracker ~= 0) and
		(self.shape:getWorldPosition() - trackertrackers[closestmatchingtracker]:getTrackerShape().worldPosition):length() or
		math.huge
...
function predictmove_super(self, mypos, direction, targetpos) -- weird shit by mathematicial guy
	...
	local t = (-3 * bb + 3 ^ (1 / 2) * aa * ((3 * bb ^ 2 - 8 * aa * cc + 4 * 2 ^ (2 / 3) * aa * cubeROOT) /
		...
		aa * (-1 * ((-3 * aa * bb ^ 2 + 8 * aa ^ 2 * cc + 2 * 2 ^ (2 / 3) * aa ^ 2 * cubeROOT + (
			...
			3 * bb ^ 2 - 8 * aa * cc + 4 * 2 ^ (2 / 3) * aa * cubeROOT) / aa ^ 2) ^ (1 / 2)) / aa ^ 3)) ^ (1 / 2)) /
	(12 * aa)
